                  ...
                  @foreach($categories as $category)
                    <div class="d-flex align-items-center justify-content-between mb-12">
                      <div class="cus-checkBox">
                        <input type="checkbox" id="cat_{{ Str::slug($category['name']) }}" class="inp-cbx category-checkbox"
                          name="category[]" value="{{ $category['name'] }}" {{ in_array($category['name'], $selected_categories ?? []) ? 'checked' : '' }}>
                        <label for="cat_{{ Str::slug($category['name']) }}"
                          class="cbx black">{{ $category['name'] }}</label>
                      </div>
                      <p>({{ $category['count'] }})</p>
                    </div>
                  @endforeach
          ...
          @if($total_pages > 1)
            <div class="pagination mt-48">
              <ul id="border-pagination">
                <li>
                  <a href="{{ route('products.index', array_merge(request()->all(), ['page' => max(1, $current_page - 1)])) }}"
                ...
                @for($i = $start_page; $i <= $end_page; $i++)
                  <li><a href="{{ route('products.index', array_merge(request()->all(), ['page' => $i])) }}"
                      class="{{ $current_page == $i ? 'active' : '' }}">{{ str_pad($i, 2, '0', STR_PAD_LEFT) }}</a></li>
                @endfor
                <li>
                  <a href="{{ route('products.index', array_merge(request()->all(), ['page' => min($total_pages, $current_page + 1)])) }}"
                    class="{{ $current_page == $total_pages ? 'disabled' : '' }}">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                      <path
